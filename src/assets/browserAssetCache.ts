 ...
 * Never put images here � ImageLoader expects HTMLImageElement / ImageBitmap;
...
async function putInThreeCache(url: string, response: Response): Promise<void> {
  ...
  try {
    ...
    // Cache miss is fine � loaders will fetch normally.
  }
}
 ...
 * iframe do not share memory � call on each page boot after a prior download).
 */
...
export async function ensureBrowserAssetsCached(
  ...
  if (typeof caches === 'undefined') {
    // Private mode / unsupported � still warm THREE.Cache via fetch.
    const urls = Object.keys(manifest.files);
    let done = 0;
    let fetched = 0;
    let failed = 0;
